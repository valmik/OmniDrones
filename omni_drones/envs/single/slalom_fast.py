...
from ..utils.traj_utils import find_closest_point_on_trajectory, find_closest_point_on_trajectory_brute_force
...
class SlalomFast(IsaacEnv):
    r"""
    Speed task. The goal is to fly along a slalom course as fast as possible.
    ...
    def __init__(self, cfg, headless):
        ...
        self.reward_progress_weight = cfg.task.reward_progress_weight
        ...
        self.nominal_target_pos = torch.zeros(self.num_envs, self.future_traj_steps, 3, device=self.device)

        self.traj_prog_tensordict = TensorDict(
            {
                "traj_progress": torch.zeros(self.num_envs, device=self.device),
            },
            self.batch_size,
        )
        self.progress = self.traj_prog_tensordict["traj_progress"]
    ...
    def _set_specs(self):
        ...
        traj_spec = CompositeSpec({
            ...
            "nominal_target_position": UnboundedContinuousTensorSpec((1, 3)),
    ...
    def _reset_idx(self, env_ids: torch.Tensor):
        ...
        pos, _ = self._compute_traj(2, add_t0=True)
        pos = pos[env_ids, 0, :]
        ...
        if self._should_render(0) and (env_ids == self.central_env_idx).any() :
            ...
            traj_vis, _ = self._compute_traj(self.max_episode_length, self.central_env_idx.unsqueeze(0), add_t0=True)
            traj_vis = traj_vis[0]
    ...
    def _compute_state_and_obs(self):
        ...
        self.last_progress = self.progress.clone()
        nominal_progress = self.last_progress + self.dt
        updated_progress = self._update_traj_time(self.progress)
        
        self.progress = self.progress_buf * self.dt
        self.nominal_target_pos[:], self.nominal_future_times = self._compute_traj(self.future_traj_steps, step_size=1)

        self.progress = torch.max(nominal_progress, updated_progress)
    ...
    def _compute_reward_and_done(self):
        # progress reward
        reward_progress = self.reward_progress_weight * (self.progress - self.last_progress)
        reward_progress = reward_progress.unsqueeze(-1)
        ...
        self.traj_stats["nominal_target_position"].copy_(self.nominal_target_pos[:, [0]])
        ...
        reward = (
            reward_pos
            + reward_pos * reward_progress
            + reward_pos * (reward_up + reward_spin)
            + reward_effort
            + reward_action_smoothness
        )

        assert self.progress.shape == self.last_progress.shape
        assert reward_progress.shape == reward_pos.shape

        self.stats["reward_progress"].add_(reward_progress)
    ...
    def _compute_traj(self, steps: int, env_ids=None, step_size: float=1., add_t0: bool=False):
        assert steps > 1
        if env_ids is None:
            env_ids = ...
        
        t = self.progress[env_ids].unsqueeze(1) + step_size * torch.arange(steps, device=self.device) * self.dt
        ...
        if len(self.ref) == 1:
            ...
            if add_t0:
                t_1 = self.traj_t0[0] + t
                t_2 = self.traj_t0[1] + t
            else:
                t_1 = t
                t_2 = t
            ...
            if add_t0:
                t = traj_t0.unsqueeze(1) + t
            target_pos = sum(one_hot[:, i].unsqueeze(-1).unsqueeze(-1) * self.ref[i].batch_pos(t) for i in range(len(self.ref)))

        return target_pos, t

    def _update_traj_time(self, t_current: torch.Tensor, env_ids=None):
        if env_ids is None:
            env_ids = ...

        if len(self.ref) == 1:
            t = find_closest_point_on_trajectory_brute_force(self.ref[0], self.drone_state[..., :3])
        elif len(self.ref) == 2:
            t_1 = find_closest_point_on_trajectory_brute_force(self.ref[0], self.drone_state[..., :3])
            t_2 = find_closest_point_on_trajectory_brute_force(self.ref[1], self.drone_state[..., :3])
            t = t_1 * (1 - self.ref_style_seq[env_ids]) + t_2 * self.ref_style_seq[env_ids]
        else:
            one_hot = torch.nn.functional.one_hot(self.ref_style_seq[env_ids], len(self.ref)).float()
            t = sum(one_hot[:, i] * find_closest_point_on_trajectory_brute_force(self.ref[i], self.drone_state[..., :3]) for i in range(len(self.ref)))
